...
def valid_cl(enc,rep,dec,M,batch,sigma,flag):
    ...
    x_re = enc_sig[:, 0,:].to("cpu").detach().numpy()
    x_im = enc_sig[:, 1,:].to("cpu").detach().numpy()
    n_re = noisy1[:, 0,:].to("cpu").detach().numpy()
    n_im = noisy1[:, 1,:].to("cpu").detach().numpy()
    m_re = mid_sig[:, 0,:].to("cpu").detach().numpy()
    m_im = mid_sig[:, 1,:].to("cpu").detach().numpy()
    y_re = noisy2[:, 0,:].to("cpu").detach().numpy()
    y_im = noisy2[:, 1,:].to("cpu").detach().numpy()
